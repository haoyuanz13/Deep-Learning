...
def generator_resnet(image, options, reuse=False, nineBlock=True, name="generator_resnet"):
  ...
  with tf.variable_scope(name):
    ...
    c1 = tf.nn.relu(instance_norm(conv2d(c0, options.gf_dim/2, 7, 7, 1, 1, padding='VALID', name='g_e1_c'), 'g_e1_bn'))

    # d64 layer, d1 is (N x 128 x 128 x 64)
    d1 = tf.nn.relu(instance_norm(conv2d(c1, options.gf_dim, 3, 3, 2, 2, name='g_e2_c'), 'g_e2_bn'))

    # d128 layer, d2 is (N x 64 x 64 x 128)
    d2 = tf.nn.relu(instance_norm(conv2d(d1, options.gf_dim*2, 3, 3, 2, 2, name='g_e3_c'), 'g_e3_bn'))
    
    '''
      Define G network with 6 resnet blocks
      ResNet keep the data shape (N x 64 x 64 x 128)
    '''
    r1 = residule_block(d2, options.gf_dim*2, name='g_r1')
    r2 = residule_block(r1, options.gf_dim*2, name='g_r2')
    r3 = residule_block(r2, options.gf_dim*2, name='g_r3')
    r4 = residule_block(r3, options.gf_dim*2, name='g_r4')
    r5 = residule_block(r4, options.gf_dim*2, name='g_r5')
    r6 = residule_block(r5, options.gf_dim*2, name='g_r6')

    r_out = r6
    
    # define G network with 9 resnet blocks
    if nineBlock:
      r7 = residule_block(r6, options.gf_dim*2, name='g_r7')
      r8 = residule_block(r7, options.gf_dim*2, name='g_r8')
      r9 = residule_block(r8, options.gf_dim*2, name='g_r9')
      r_out = r9
    ...
    u1 = deconv2d(r_out, [options.batch_size, s2, s2, options.gf_dim], 3, 3, 2, 2, name='g_d1_dc')
    u1 = tf.nn.relu(instance_norm(u1, 'g_d1_bn'))

    # u32 layer, u1 is (N x 256 x 256 x 32)
    u2 = deconv2d(u1, [options.batch_size, s, s, int(options.gf_dim/2)], 3, 3, 2, 2, name='g_d2_dc')
...
def discriminator(image, options, reuse=False, name="discriminator"):
  ...
  with tf.variable_scope(name):
    ...
    h3 = lrelu(instance_norm(conv2d(h2, options.df_dim*8, d_h=1, d_w=1, name='d_h3_conv'), 'd_bn3'))
    # h3 is (32 x 32 x self.df_dim*8)
    
    h4 = conv2d(h3, 1, d_h=1, d_w=1, name='d_h3_pred')
    # h4 is (32 x 32 x 1)
    return h4